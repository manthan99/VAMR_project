...
if ds == 2
    ...
    query_harris = harris(img0, harris_patch_size, harris_kappa);
...
query_harris = harris(prev_img, harris_patch_size, harris_kappa);
corners0 = selectKeypoints(...
    query_harris, num_keypoints, nonmaximum_supression_radius);
corners0 = flipud(corners0)';
...
%     % If using Estimate World Camera Pose
    [R,T, inlierIDX] = estimateWorldCameraPose...
        (points1,worldPoints,intrinsics, 'Confidence', 95, 'MaxReprojectionError', 3, 'MaxNumTrials', 1000);
...
%     hold on
    ...
    tracker = vision.PointTracker('MaxBidirectionalError',1);
    initialize(tracker,prev_state.D,prev_img);
...
%             if(T == T1 | end_ind - start_ind < 10)
    ...
    frame = plot_screencast(prev_img, prev_state.P, points_plot, prev_state.n_landmark, prev_state.X, prev_state.Trajectory);
    pause(0.1);
    T_viz(1);
