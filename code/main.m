...
%     [prev_features, prev_pts] = extractFeatures(prev_img, inlierCurrPts);
    ...
    initialize(tracker, inlierCurrPts, prev_img);
    [points1, KLT_inliers, KLT_scores] = tracker(query_image);
    sum(KLT_inliers)/length(KLT_inliers)
    points0 = inlierCurrPts(KLT_inliers,:);
...
%     currMatchedPts = curr_pts(indexPairs(:,2),:);
    ...
    worldPoints = worldPoints(KLT_inliers,:);
    
    figure(3),
    showMatchedFeatures(prev_img,query_image,points0,points1, 'Montage');
    legend('matched points 1','matched points 2');
%     M = estimatePoseDLT(prevMatchedPts.Location, worldPoints_curr, K);
%     R = M(1:3,1:3)
%     T = M(1:3,4)
    [worldOrientation,worldLocation, inlierIDX] = estimateWorldCameraPose(points1,worldPoints,intrinsics);
    inlierIDX = find(inlierIDX);
    figure(2),
    plotCamera('Size',1,'Orientation',worldOrientation,'Location',...
     worldLocation);
%     hold on
    inlierCurrPts = points1;
